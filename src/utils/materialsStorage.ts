  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.supabaseError) {
        console.warn(
          'Supabase database sync issue (non-blocking):',
          data.supabaseError
        );
        // Save database error state so the admin dashboard can show a helpful warning banner
        localStorage.setItem('nile_supabase_sync_error', data.supabaseError.message || JSON.stringify(data.supabaseError));
      } else {
        localStorage.removeItem('nile_supabase_sync_error');
      }
