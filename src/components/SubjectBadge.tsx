...
export const TaskTypeBadge: React.FC<{ type?: string; className?: string }> = ({ type, className = '' }) => {
  const isHomework = type === 'homework' || type === 'واجب';
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold ${
        isHomework ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800'
      } ${className}`}
    >
      {type || 'Task'}
    </span>
